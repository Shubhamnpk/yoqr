...
export default function QRScanner({
  ...
  useEffect(() => {
    ...
    const initScanner = async () => {
      try {
        ...
        // Only import the library when the component is mounted
        const { Html5Qrcode } = await import('html5-qrcode');
        
        // Make sure the component is still mounted and the reader element exists
  ...
  useEffect(() => {
        ...
        try {
          ...
          await scannerRef.current.start(
            ...
            (decodedText: string, decodedResult: any) => {
              // Wrap callback in try-catch to prevent React errors
              try {
    ...
    startScan();
  }, [isScanning, currentCamera, scanMode, setIsScanning, toast]);
